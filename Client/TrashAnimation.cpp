...
std::atomic<bool> TrashAnimation::load_complete = false;
...
void TrashAnimation::Start()
{
	...
	std::thread loading_thread(&TrashAnimation::LoadSpriteAsync, this);
}
...
void TrashAnimation::LoadSpriteAsync()
{
	while (load_complete == false)
	{
		...
		if (IsLoadMaxIndex())
		{
			ResourceManager::AddImageByPath(pluskey + key + " (" + std::to_string(loadIndex) + ").png", sstream.str().c_str(), 1.0f);
			//ResourceManager::AddImageByPath(key + std::to_string(loadIndex), sstream.str().c_str(), 1.0f);
		}
		else
		{
			ResourceManager::AddImageByPath(pluskey + key + " (" + std::to_string(loadIndex) + ").png", sstream.str().c_str(), quality);
			//ResourceManager::AddImageByPath(key + std::to_string(loadIndex), sstream.str().c_str(), quality);
		}
	}
}
